...
class User(Model):
    ...
    def get_total_count_qoutes(self, user_id):

        query = "SELECT COUNT(quotes.message) as count, users.name FROM quotes JOIN users ON users.id=quotes.user_id WHERE users.id=:user_id"
        data = {'user_id': user_id}
        return self.db.query_db(query, data)

    def get_all_quotes(self, user_id):
        
        query = "SELECT quotes.message, quotes.quoted_by FROM quotes WHERE quotes.user_id=:user_id"
        data =  {'user_id':user_id}
        return self.db.query_db(query,data)
